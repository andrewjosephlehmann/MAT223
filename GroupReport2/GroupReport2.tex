...
\begin{figure}
	...
	\caption{The path traced light in $\mathbb{R}^3$ that depicts $\vec{b}$ as a sphere and set $A$ in red as a two-dimensional subspace.}
...
In this duality, mathematically breaking down a projection is far more approachable. There are multiple means of doing this in $\mathbb{R}^3$, but in our case, in $\mathbb{R}^2$, it is much more trivial since $\vec{b}$ is a point in space. Yet, the critical concept remains constant, and it is a matter of making the sphere flush with the two-dimensional subspace. With that, we may assume that the sphere is an element of the subspace. Akin to what breaks down a projection in $\mathbb{R}^2$. }
